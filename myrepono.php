...
function myrepono_get_status() {
	...
	if ($myrepono_array = get_option("myrepono")) {
		...
		if (!isset($myrepono_array['myr_cache'])) {
			...
			if ($myrepono_array['myr_cache']['last_refresh']<(time()-300)) {
				$refresh_cache = "1";
			}
		} else {
			$refresh_cache = "1";
...
<br>&nbsp;<br>
Useful Links: <a href="http://myRepono.com/" target="new" class="button-primary"><b>myRepono.com</b></a> <a href="http://myRepono.com/faq/" target="new" class="button-primary"><b>FAQ &amp; Documentation</b></a> <a href="http://myRepono.com/contact/" target="new" class="button-primary"><b>Contact Us</b></a>
</p>
END;
...
END;
		...
		if ($myrepono_submit=="Confirm Reset Default Configuration") {
			...
			if (($myrepono_input_email!="") && ($myrepono_input_password!="")) {
				...
				if (isset($myrepono_hapi['result_source'][0])) {
					...
					if ($myrepono_hapi['result_source_response'][0]!="1") {
						...
						$myrepono_response = "<img src=\"{$myrepono_plugin_url}img/accept.png\" width=\"16\" height=\"16\" alt=\"\" style=\"position:relative;top:2px;\">&nbsp; Login Successful!";
		...
		if (($myrepono_array['myr_username']=="") || ($myrepono_array['myr_password']=="")) {
			...
			if ($myrepono_free_trial_topup_credit = file_get_contents("http://myRepono.com/sys/free_trial_credit/")) {
				...
				print<<<END
<div class="myrepono_response"><p><img src="{$myrepono_plugin_url}img/error.png" width="16" height="16" alt="" style="position:relative;top:2px;">&nbsp; <b>Warning:</b> Initial tests demonstate this plugin is unable to connect to myRepono.com, therefore the create account process may not work correctly.<p></div>
END;
...
END;
			...
			if ($refresh_cache=="1") {
				...
				print<<<END
<div class="myrepono_response"><p><img src="{$myrepono_plugin_url}img/error.png" width="16" height="16" alt="" style="position:relative;top:2px;">&nbsp; <b>Warning:</b> Invalid myRepono account details set, please select the 'Reset Default Configuration' button in the bottom right and re-login.<p></div>
END;
...
<tbody>
   <tr>
	...
	<p>Your myRepono plugin is currently configured for your myRepono.com account, <b>$myrepono_username</b>.  Please select the button below to clear your current configuration, this will not disrupt your backups which will remain stored in your account on myRepono.com.</p>
	<form name="form" action="options-general.php?page=myrepono" method="post">
...
function myrepono_css() {
	...
	.myrepono_response {
border-width:1px;border-style:solid;padding:0 .6em;margin:5px 15px 2px;-moz-border-radius:3px;-khtml-border-radius:3px;-webkit-border-radius:3px;border-radius:3px;background-color:#ffffe0;border-color:#e6db55;
	}
	</style>
	";
}
