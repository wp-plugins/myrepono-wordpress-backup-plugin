...
function myrepono_get_status() {
	...
	if ($myrepono_array = get_option("myrepono")) {
		...
		if (!isset($myrepono_array['myr_cache'])) {
			...
			if ($myrepono_array['myr_cache']['last_refresh']<(time()-1800)) {
				$refresh_cache = "1";
			}
		} else {
			$refresh_cache = "1";
		...
		} elseif ($myrepono_array['myr_username']=="") {
			$refresh_cache = "0";
		}

		if (($refresh_cache=="1") || ($myrepono_array['myr_username']!="")) {
		...
		if ($myrepono_array['myr_username']!="") {

			$myrepono_plugin_path = dirname(__FILE__);
			$myrepono_plugin_path = str_replace('\\','/',$myrepono_plugin_path);
			$myrepono_plugin_api_path = $myrepono_plugin_path."/api/";
			$myrepono_plugin_api_url = $myrepono_plugin_url."api/";

			if ((file_exists($myrepono_plugin_api_path)) && (!file_exists($myrepono_plugin_api_path."myrepono.php"))) {
				if (($myrepono_array['myr_username']!="") && ($myrepono_array['myr_password']!="") && ($myrepono_array['myr_domain_id']!="")) {

					$myrepono_api_reinstalled = "0";

					$myrepono_hapi['request_url'] = "https://myrepono.com/hapi/h.api?api_key=".rawurlencode($myrepono_hapi_input['api_key'])."&api_pass=".rawurlencode($myrepono_hapi_input['api_pass']);

					$myrepono_hapi['request_url_source'] = $myrepono_hapi['request_url']."&api_action=source&email=".rawurlencode($myrepono_array['myr_username'])."&password=".rawurlencode($myrepono_array['myr_password'])."&domain_id=".rawurlencode($myrepono_array['myr_domain_id']);

					$myrepono_hapi['result_source'] = myrepono_connect($myrepono_hapi['request_url_source']);
					...
					if (isset($myrepono_hapi['result_source'][0])) {
						...
						if ($myrepono_hapi['result_source_response'][0]!="1") {
							...
							if (isset($myrepono_hapi['result_source'][1])) {
								...
								$myrepono_hapi_input['domain_path_myrepono'] = $myrepono_plugin_api_path."myrepono.php";
					...
					if ($myrepono_api_reinstalled=="0") {

						$myrepono_output = "<img src=\"{$myrepono_plugin_url}img/myrepono_icon.png\" width=\"21\" height=\"12\" style=\"position:relative;bottom:-3px;padding-right:2px;\"> <a href=\"options-general.php?page=myrepono\">myRepono Plugin Error - Click Here</a>";
						return $myrepono_output;

					} elseif ($myrepono_api_reinstalled=="1") {

						$myrepono_output = "<img src=\"{$myrepono_plugin_url}img/myrepono_icon.png\" width=\"21\" height=\"12\" style=\"position:relative;bottom:-3px;padding-right:2px;\"> <a href=\"options-general.php?page=myrepono\">myRepono API Updated</a>";
						return $myrepono_output;
...
function myrepono() {
	...
	if ($myrepono_array = get_option("myrepono")) {
		if (!isset($myrepono_array['myr_show_status'])) {
			$myrepono_array['myr_show_status'] = "1";
			update_option("myrepono", $myrepono_array);
		} else {
			$myr_show_status = $myrepono_array['myr_show_status'];
		}
	}

	$myrepono_status = myrepono_get_status();

	if ($myr_show_status=="1") {
...
<a href="http://myRepono.com/" target="new"><img src="{$myrepono_plugin_url}img/myrepono_logo_grey_200.png" width="200" height="55" border="0" align="right" hspace="10" vspace="0" alt="myRepono WordPress Backup Service" title="myRepono WordPress Backup Service" id="myrepono_logo"></a>
...
END;
		...
		if ($myrepono_submit=="View Domain") {
			...
			$myrepono_hapi['request_url_source'] = $myrepono_hapi['request_url']."&api_action=domain-settings&email=".rawurlencode($myrepono_array['myr_username'])."&password=".rawurlencode($myrepono_array['myr_password'])."&domain_id=".rawurlencode($myrepono_array['myr_domain_id'])."&set_active=0";
			...
			if ($myrepono_settings_saved=="1") {
				...
				$myrepono_response = "<img src=\"{$myrepono_plugin_url}img/accept.png\" width=\"16\" height=\"16\" alt=\"\" style=\"position:relative;top:2px;\">&nbsp; <b>Your myRepono WordPress Backup Plugin has been reset.</b><br>&nbsp;<br>When you setup the plugin a domain configuration was created in your myRepono.com account, this domain configuration has not been removed but it has been paused to stop further backups.<br>&nbsp;<br>If you now setup the plugin again you will create a second domain configuration in your account and you will replace your existing API, this can become confusing because you will have two domain configurations with the same name - the first will no longer work correctly (and will have been automatically paused) and should be removed via <A href=\"https://myRepono.com/my/\" target=\"new\"><b>myRepono.com</b></a> (this is not automated in case of accidental backup removal).<br>&nbsp;<br>If you have any questions, please <A href=\"https://myRepono.com/contact/new/\" target=\"new\"><b>contact support</b></a>.";

			} else {

				$myrepono_response = "<img src=\"{$myrepono_plugin_url}img/accept.png\" width=\"16\" height=\"16\" alt=\"\" style=\"position:relative;top:2px;\">&nbsp; <b>Your myRepono WordPress Backup Plugin has been reset.</b><br>&nbsp;<br>When you setup the plugin a domain configuration was created in your myRepono.com account, this domain configuration has not been removed and will continue to backup your website even though it is no longer linked with your plugin.<br>&nbsp;<br>If you now setup the plugin again you will create a second domain configuration in your account and you will replace your existing API, this can become confusing because you will have two domain configurations with the same name - the first will no longer work correctly and should be removed via <A href=\"https://myRepono.com/my/\" target=\"new\"><b>myRepono.com</b></a> (this is not automated in case of accidental backup removal).<br>&nbsp;<br>If you have any questions, please <A href=\"https://myRepono.com/contact/new/\" target=\"new\"><b>contact support</b></a>.";
			...
			if (($myrepono_input_email!="") && ($myrepono_input_password!="")) {
				...
				if (isset($myrepono_hapi['result_source'][0])) {
					...
					if ($myrepono_hapi['result_source_response'][0]!="1") {
						...
						if ($myrepono_input_login_authenticate=="1") {
							...
							$myrepono_hapi_input['domain_database_tables'] = myrepono_hapi_dbtables_array($myrepono_hapi_input);
							...
							if ($myrepono_hapi_response['result']=="1") {
								...
								if (file_exists($myrepono_plugin_api_path."myrepono.php")) {
									...
									if ($myrepono_api_response = myrepono_connect($myrepono_plugin_api_url."myrepono.php")) {
				...
				if ($myrepono_login_successful!="1") {
					if ($myrepono_hapi['result_source_response'][2]=="User Not Active") {
						$myrepono_hapi['result_source_response'][2] = "Account is inactive, please ensure you have completed the email activation and choose password stages.";
					} else {
			...
			if (($myrepono_input_currency=="USD") || ($myrepono_input_currency=="GBP") || ($myrepono_input_currency=="EUR")) {
				if ($myrepono_input_email=="") {
					...
					if ($myrepono_hapi_response['result']=="1") {
						...
						$myrepono_response = "<img src=\"{$myrepono_plugin_url}img/accept.png\" width=\"16\" height=\"16\" alt=\"\" style=\"position:relative;top:2px;\">&nbsp; <b>Sign-Up Completed Successfully!</b><br>Log-In Email: ".$myrepono_hapi_input['email']."<br><b>Please Check Your Email!</b><br>An activation link has been sent via email, please select this link to activate your account and choose your password, once done please enter your login password below.";
		...
		if (($myrepono_array['myr_username']=="") || ($myrepono_array['myr_password']=="")) {
			...
			if ($myrepono_free_trial_topup_credit = myrepono_connect("https://myRepono.com/sys/free_trial_credit/")) {
				...
				print<<<END
<div class="myrepono_response"><p><img src="{$myrepono_plugin_url}img/error.png" width="16" height="16" alt="" style="position:relative;top:2px;">&nbsp; <b>Warning:</b> Initial tests demonstate this plugin is unable to connect to myRepono.com, therefore the create account and log-in processes may not work correctly.  Please contact support for assistance.<p></div>
END;
...
<p>To begin, please enter your email address to sign-up for a myRepono.com account.  Registration is free, no payment details are required until you choose to top-up your balance (after your free trial). All information is stored with the strictest confidence, for more information please see our <A href="https://myrepono.com/privacy/" target="new"><b>Privacy Policy</b></a>.</p>
	...
	<tr>
		...
		<td>
			...
			<input name="myrepono_terms" id="myrepono_terms" value="1" type="checkbox"$myrepono_terms_checked>
			 I have read, understand and agree to the <A href="https://myrepono.com/terms/" target="new"><b>Terms of Service</b></a>.</label>
		</td>
	</tr></tbody></table>
...
<table class="form-table">
	...
	<tr>
		...
		<td>
			<input name="myrepono_password" id="myrepono_password" value="" type="password" class="regular-text code"> <a href="https://myRepono.com/password/" target="new"><b>Lost your password?</b></a>
		</td>
	</tr></tbody></table>
...
<tbody>
   <tr>
	...
	<p>myRepono is a remote website backup service which enables you to backup your WordPress and website files and databases.</p>
	...
	<p>You can store up to 750MB of backups, and backup up to 75MB per day and only pay \$0.02 USD per day!</p>

	<p>No payment details are required until you choose to top-up your balance.</p>
...
END;
			...
			//$refresh_cache = "1";
			//$myrepono_array['myr_cache']['last_refresh'] = "0";
			//$myrepono_array['myr_cache']['last_refresh_delay'] = "0";

			if (!isset($myrepono_array['myr_cache'])) {
				$refresh_cache = "1";
			} elseif (isset($myrepono_array['myr_cache']['last_refresh'])) {
				if ($myrepono_array['myr_cache']['last_refresh']<(time()-120)) {
					$refresh_cache = "1";
				}
			} else {
				$refresh_cache = "1";
...
<table class="form-table">
	...
	<tr>
		...
		<td>
			<input name="myrepono_password" id="myrepono_password" value="" type="password" class="regular-text code"><br><a href="https://myRepono.com/password/" target="new"><b>Lost your password?</b></a>
		</td>
	</tr></tbody></table>
					...
					if (isset($myrepono_hapi['result_source'][0])) {
						...
						if ($myrepono_hapi['result_source_response'][0]!="1") {
							...
							if ($myrepono_hapi['result_source_response'][2]>0) {
								...
								if ($myrepono_domain_id_matched!="1") {

									$myrepono_domain_id = $myrepono_array['myr_cache']['domains'][0]['id'];
									$myrepono_array['myr_domain_id'] = $myrepono_domain_id;
			...
			if ((file_exists($myrepono_plugin_api_path)) && (!file_exists($myrepono_plugin_api_path."myrepono.php"))) {
				if (($myrepono_array['myr_username']!="") && ($myrepono_array['myr_password']!="") && ($myrepono_array['myr_domain_id']!="")) {
					...
					if ($myrepono_api_reinstalled=="0") {
						...
						print<<<END
<div id="message" class="updated"><p><img src="{$myrepono_plugin_url}img/error.png" width="16" height="16" alt="" style="position:relative;top:2px;">&nbsp; <b>Warning:</b> myRepono API does not exist and plugin is unable to re-install API.  Your options are as follows:<br>
<ul>
<li>1. Manually re-install the API by downloading it via your myRepono account (see 'Domains' -&gt; 'API' -&gt; 'Manual API Installation' section).</li>
<li>2. Select the 'Reset Default Configuration' option and re-configure your myRepono WordPress Backup Plugin.</li>
<li>3. Contact myRepono support for assistance.</li>
</ul><p></div>
END;
						...
						print<<<END
<div id="message" class="updated"><p><img src="{$myrepono_plugin_url}img/error.png" width="16" height="16" alt="" style="position:relative;top:2px;">&nbsp; <b>Note:</b> Your myRepono API has been automatically re-installed (it is removed during the plugin update process).<p></div>
END;
			...
			if (file_exists($myrepono_plugin_api_path."myrepono_config.php")) {
				if (($myrepono_array['myr_username']!="") && ($myrepono_array['myr_password']!="") && ($myrepono_array['myr_domain_id']!="")) {
					$myrepono_array['myr_api_config'] = @file_get_contents($myrepono_plugin_api_path."myrepono_config.php");
					update_option("myrepono", $myrepono_array);
				}
			} elseif (isset($myrepono_array['myr_api_config'])) {
				if ($myrepono_array['myr_api_config']!="") {
					$myrepono_config_file = @fopen($myrepono_plugin_api_path."myrepono_config.php", 'w');
					if ($myrepono_config_file) {
						fwrite($myrepono_config_file, $myrepono_array['myr_api_config']);
						fclose($myrepono_config_file);
						print<<<END
<div id="message" class="updated"><p><img src="{$myrepono_plugin_url}img/error.png" width="16" height="16" alt="" style="position:relative;top:2px;">&nbsp; <b>Note:</b> Your 'myrepono_config.php' API configuration file has been automatically re-installed (it is removed during the plugin update process).<p></div>
END;
...
END;
			...
			//print "<p><pre style=\"font-size:9px;line-height:11px;\">".print_r($myrepono_array,1)."</pre></p>";
			...
			for ($i=1;$i<51;$i++) {
				...
				if ($i==$backup_stored) {
					$myrepono_select_stored .= " selected=\"selected\"";
				}
				$myrepono_select_stored .= ">$i&nbsp; </option>\n";
			}

			$stored_days_values = array("90","120","180","365","730","1095","1825","10000");

			for ($i=0; $i<count($stored_days_values); $i++) {

				$myrepono_select_stored .= "<option value=\"".$stored_days_values[$i]."\"";
				if ($backup_stored==$stored_days_values[$i]) {
					$myrepono_select_stored .= " selected=\"selected\"";
				}
				$myrepono_select_stored .= ">".$stored_days_values[$i]."</option>\n";
			...
			if ($backup_status=="1") {
				$display_status1_checked = " selected=\"selected\"";
			} else {
				$display_status0_checked = " selected=\"selected\"";
			}
...
END;
			...
			for ($i=1;$i<31;$i++) {
				...
				if ($backup_frequency_days==$i) {
					$myrepono_select_frequency .= " selected=\"selected\"";
				}
			...
			for ($i=0;$i<$location_count;$i++) {
				...
				if ($backup_primary_location==$location_key) {
					$myrepono_select_primary_location .= " checked=\"checked\"";
				}
				...
				if ($location_count>1) {
					...
					for ($j=0;$j<count($backup_mirror_location_explode);$j++) {
						if ($backup_mirror_location_explode[$j]==$location_key) {
							$myrepono_select_mirror_location .= " checked=\"checked\"";
						}
					}
			...
			if (isset($myrepono_array['myr_cache']['backup'])) {
				...
				for ($i=0; $i<$myrepono_backup_count; $i++) {
					...
					if (isset($myrepono_array['myr_cache']['backup'][$myrepono_backup_key]['id'])) {
						...
						if (!is_numeric($myrepono_backup_packets)) {
							$myrepono_backup_packets = "1";
						}

						if ($myrepono_backup_packets=="1") {
							$myrepono_backup_packets .= " Packet";
						} else {
							$myrepono_backup_packets .= " Packets";
						}

						if ($myrepono_last_backup_date!=$myrepono_backup_date) {

							$myrepono_last_backup_date = $myrepono_backup_date;

							$myrepono_backups .= <<<END
   <tr>
     <td colspan="5" style="background:#f0f0f0;"><b><i>$myrepono_backup_date</i></b></td>
   </tr>
END;
   ...
   <tr>
     ...
     <td nowrap><b style="font-size:13px;">$myrepono_backup_time</b></td>
     ...
     <td nowrap><a href="https://myRepono.com/my/backups/view/$myrepono_backup_id/" class="button-primary" target="new">View</a> <a href="https://myRepono.com/my/backups/restore/$myrepono_backup_id/" class="button-primary" target="new">Restore</a></td>
   </tr>
...
<br>
<p><small>Note: All data is sourced from myRepono.com, as such page loading may be delayed when caching data.  Data is cached for 2 or 5 minutes depending on importance, this means it may take up to 5 minutes for changes to appear.</small></p>
END;

				if (file_exists($myrepono_plugin_api_path."/myrepono.php")) {
					print<<<END
<p><small><b>Rate the myRepono WordPress Backup Plugin</b><br>
Please help support the myRepono WordPress Backup Plugin by <A href="http://wordpress.org/extend/plugins/myrepono-wordpress-backup-plugin/" target="new"><b>rating our plugin on WordPress.org</b></a>.
<br><i>Thank you for using myRepono!</i></small></p>
END;
				}

				if (isset($_GET["myr_status_box"])) {
					if ($_GET["myr_status_box"]=="1") {
						$input_myr_status_box = "1";
					} else {
						$input_myr_status_box = "0";
					}
					$myrepono_array['myr_show_status'] = $input_myr_status_box;
					$myr_show_status = $input_myr_status_box;
					update_option("myrepono", $myrepono_array);
				}
				...
				if ($myr_show_status=="1") {
					$myr_show_status_txt = "<b>Enabled <A href=\"options-general.php?page=myrepono&myr_status_box=0\">[Disable Status Box]</a></b>";
				} else {
					$myr_show_status_txt = "<b>Disabled <A href=\"options-general.php?page=myrepono&myr_status_box=1\">[Enable Status Box]</a></b>";
				}


				print<<<END
<p><small><b>Plugin Status Box</b><br>
The plugin status box is shown in the WordPress administration panel header and informs you when your last backup was processed.  The plugin status box is currently: $myr_show_status_txt
</small></p>
...
<tbody>
   <tr>
     <td>
		...
		<p class="submit" style="margin-bottom:6px;padding-bottom:0px;">
			<a href="https://myRepono.com/my/billing/topup/" class="button-primary" target="new">Top-Up Balance</a>
			<a href="https://myRepono.com/my/" class="button-primary" target="new">View Account</a>
		</p>
...
END;
			...
			print<<<END
<div class="wrap">
<p><b>An error has occurred whilst trying to setup your myRepono configuration data, please <a href="https://myRepono.com/contact/">contact support</a>.</b></p>
</div>
END;
...
function myrepono_css() {
	...
	$myr_status_top = "4.7em";
	$myr_status_right = "215px";
	...
	print <<<END
	<style type="text/css">
#myrepono_status{position:absolute;top:$myr_status_top;margin:0;padding:4px;padding-left:8px;padding-right:8px;right:$myr_status_right;font-size:10px;z-index:2;background:#757575;color:#fff;-moz-border-radius:0 0 5px 5px;-webkit-border-radius:0 0 5px 5px;}#myrepono_status a{color:#fff;text-decoration:none;position:relative;top:-1px;}#myrepono_logo{padding:2px;border:1px solid #bbb;margin-left:10px;margin-right:10px;}.myrepono_response{border-width:1px;border-style:solid;padding:0 .6em;margin:5px 15px 2px;-moz-border-radius:3px;-khtml-border-radius:3px;-webkit-border-radius:3px;border-radius:3px;background-color:#ffffe0;border-color:#e6db55;}
	</style>
END;
...
function myrepono_connect($request_url = "") {

	if ($request_response = @file_get_contents($request_url)) {

		return $request_response;

	} else {

		if (file_exists(dirname(__FILE__)."/api/libcurlemu/libcurlemu.inc.php")) {
			require_once(dirname(__FILE__)."/api/libcurlemu/libcurlemu.inc.php");
		}
		...
		if ($ch) {
			...
			$request_url = str_replace("https://myrepono.com/", "http://myrepono.com/", $request_url);
...
function myrepono_hapi_auto_setup($myrepono_hapi_input) {
	...
	if ($myrepono_hapi_input['skip_signup']!="1") {
		...
		$myrepono_hapi['result_signup'] = myrepono_connect($myrepono_hapi['request_url_signup']);
		...
		if (isset($myrepono_hapi['result_password'])) {
			if ($myrepono_hapi['result_password']!="") {
				...
				$myrepono_hapi['result_domain'] = myrepono_connect($myrepono_hapi['request_url_domain']);
...
function myrepono_hapi_dbtables_array($myrepono_hapi_input = array()) {

	$output = array();

	if (isset($myrepono_hapi_input['domain_database_host'])) {
		if ($myrepono_hapi_input['domain_database_host']!="") {


			mysql_connect($myrepono_hapi_input['domain_database_host'], $myrepono_hapi_input['domain_database_user'], $myrepono_hapi_input['domain_database_pass']);
			mysql_select_db($myrepono_hapi_input['domain_database_name']);
