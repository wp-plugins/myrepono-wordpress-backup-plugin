 ...
 */
/*
Copyright 2012 ionix Limited (email: support@myRepono.com)
...
function myrepono_plugin_init($tab = '', $init_request = false, $refresh_cache = '0', $status_box = '0') {
	...
	if ($myrepono = myrepono_get_option()) {
		...
		if (($myrepono['plugin']['url']!=$myrepono_plugin_url) || ($myrepono['plugin']['path']!=$myrepono_plugin_path)) {
			...
			$myrepono['tmp']['error'][] = 'Your plugin is currently installed incorrectly, please disconnect the plugin and then repeat the \'Connect Plugin\' process.';
	...
	if ((isset($myrepono['papi']['connect']['auth_error'])) && ($myrepono['papi']['connect']['auth_error']>0)) {
...
<div class="myrepono_setup">
	...
	<p><a href="https://myRepono.com/papi/1.0/auth/begin/?plugin=wordpress&secret=$myrepono_secret&return=$myrepono_return_url&api=$myrepono_api_url" onclick="return myrepono_authorise(this.href);" class="button-primary">Connect Plugin</a></p>
...
function myrepono_plugin_begin_setup() {
	...
	if ($db_test = new wpdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST)) {
		...
		if ($db_test_result = $db_test->get_results('show tables from `'.mysql_escape_string(DB_NAME).'`;', ARRAY_A )) {
...
function myrepono_connect($request = array(), $request_uri = false) {
	...
	if ($request_uri===false) {
		...
		if (($myrepono['papi']['connect']['put']=='0') || ($myrepono['papi']['connect']['put']=='1')) {
			...
			if ($request_encode!='') {
				...
				$response = myrepono_connect_put($request_url, $request_encode);
				...
				if (($response===false) && ($myrepono['papi']['connect']['protocol']=='0')) {
					...
					$response = myrepono_connect($request);
	...
	if (($response===false) || ($myrepono['papi']['connect']['put']=='2')) {
		...
		$response = myrepono_connect_url($request_url);
...
function myrepono_connect_url($request_url, $timeout = 60) {
...
function myrepono_connect_put($request_url, $request_encode, $timeout = 60) {
	...
	if ($output!='') {
		...
		if (($type=='') || ($type=='header')) {
			...
			if ($tabs_output!='') {
				...
				$tabs_output = '<h2 class="nav-tab-wrapper">'.$tabs_output.'<a href="http://myRepono.com/" target="new"><img src="'.$img_url.'/myrepono_logo_trans_nav.png" align="right" width="94" height="29" alt="myRepono Website Backup Service" title="myRepono Website Backup Service" border="0" style="position:relative;left:2px;" /></a></h2>';
...
END;
			...
			/*
			if (isset($_GET['myr_array'])) {

				global $myrepono;

				if ((isset($myrepono['papi']['token'])) && ($myrepono['papi']['token']!='') && ($_GET['myr_array']==substr($myrepono['papi']['token'],0,16))) {

					print "<hr><pre style=\"font-face:Georgia;font-size:9px;line-height:11px;\">".print_r($myrepono,1)."</pre><hr>";
